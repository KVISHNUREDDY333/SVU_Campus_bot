...
async def register_user(user_data: RegisterRequest):
    if not is_valid_email(user_data.email):
        logger.warning(
            f"Registration blocked for unsupported provider: {user_data.email}"
        )
        raise HTTPException(
            status_code=400,
    ...
    try:
        ...
        access_token = create_access_token(
            data={"sub": user_data.email, "role": "student"}
        )
        return {
            "access_token": access_token,
            "token_type": "bearer",
...
async def verify_otp_reset(request: VerifyOTPRequest):
    ...
    if str(record["otp"]).strip() != input_otp:
        logger.warning(
            f"Invalid OTP for {input_email}. Expected: {record['otp']}, Got: {input_otp}"
        )
        raise HTTPException(status_code=400, detail="Invalid OTP")

    new_hash = get_password_hash(request.new_password[:72])
    database.users_db.update_one(
        {"username": request.email}, {"$set": {"password_hash": new_hash}}
    )
...
async def google_id_token_login(request: Request):
    ...
    if not email or not is_valid_email(email):
        raise HTTPException(
            status_code=400,
            detail="Only supported email providers (Gmail, Outlook, Yahoo, Zoho) are allowed",
        )
    ...
    if not db_user:
        ...
        database.users_db.update_one(
            {"username": email}, {"$set": {"last_login": datetime.utcnow()}}
        )
...
async def auth_google(request: Request):
    try:
        ...
        if not email or not is_valid_email(email):
            raise HTTPException(
                status_code=400,
                detail="Only official email accounts (Gmail, Outlook, Yahoo, Zoho) are accepted.",
            )
...
async def update_profile(
    request: ProfileUpdateRequest, current_user: User = Depends(get_current_user)
):
    try:
